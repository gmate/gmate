...
class MultiEditPlugin(gedit.Plugin):
    ...
    def __init__(self):
        ...
        # Shortcuts
        self._sc_add_mark_str = self._get_config_str('add_mark', self._sc_add_mark_def)
        self._sc_level_marks_str = self._get_config_str('level_marks', self._sc_level_marks_def)
        self._sc_auto_incr_str = self._get_config_str('auto_incr', self._sc_auto_incr_def)
        
        self._sc_mark_vert_str = {
            'up':self._get_config_str('mark_up', self._sc_mark_vert_def['up']),
            'down':self._get_config_str('mark_down', self._sc_mark_vert_def['down']),
            'eol_up':self._get_config_str('mark_eol_up', self._sc_mark_vert_def['eol_up']),
            'eol_down':self._get_config_str('mark_eol_down', self._sc_mark_vert_def['eol_down']),
        }
    ...
    def _set_shortcut_keyvals(self):
        ...
        self._sc_mark_vert = {
            ...
            'eol_up':self._get_keyval(self._sc_mark_vert_str['eol_up']),
            'eol_down':self._get_keyval(self._sc_mark_vert_str['eol_down']),
        }
    ...
    def _parse_sc_auto_incr(self, string):
        ...
        result = {}
        for line in lines:
            (command, sep, values) = line.partition(':')
            if sep != ':': continue
            (key, sep, command) = command.partition(' ')
            if sep != ' ': continue
            keyval, shift_req = self._get_keyval(key)
            if keyval == 0: continue
            values = csv.reader([values]).next()
            result[keyval] = [shift_req, command]
            result[keyval].extend(values)
        return result
    ...
    def _save_settings(self):
        ...
        self._set_config_str('add_mark', self._sc_add_mark_str)
        self._set_config_str('level_marks', self._sc_level_marks_str)
        self._set_config_str('auto_incr', self._sc_auto_incr_str)
        
        self._set_config_str('mark_up', self._sc_mark_vert_str['up'])
        self._set_config_str('mark_down', self._sc_mark_vert_str['down'])
        self._set_config_str('mark_eol_up', self._sc_mark_vert_str['eol_up'])
        self._set_config_str('mark_eol_down', self._sc_mark_vert_str['eol_down'])
