...
class WindowInstance:
    ...
    def _tab_change(self, window, tab):
        ...
        if tab is not None:
            ...
            self._vert_mark_mem = None  # Filled format = (line_num, to_line_end)
            
            # Event trackers
            self._vert_mark_mem = False
    ...
    def _mouse_handler(self, event, type_):
        ...
        if type_ == 'press':
            ...
            if shift_on:
                ...
                down = abs(dif) == dif
                to_line_end = event.button == 3
                for i in range(abs(dif)):
                    self._vertical_cursor_nav(down, to_line_end)
    ...
    def _keyboard_handler(self, event):
        ...
        if ctrl_on:
            ...
            for sc in self._plugin._sc_mark_vert:
                ...
                if event.keyval == keyval and (shift_req == None or shift_req == shift_on):
                    # Recover vertical move mems, regardless if needed (since _vertical_cursor_nav() will handle it)
        ...
        if len(self._marks) != 0:
            ...
            if ctrl_on:
                ...
                if event.keyval in self._plugin._sc_auto_incr:
                    ...
                    shift_req = entry[0]
                    if shift_req == None or shift_req == shift_on:
                        values = self._auto_increment(entry[1:])
            ...
            if not ctrl_on:
                ...
                if self._view.get_auto_indent() and event.keyval == gtk.keysyms.Return and not shift_on:
    ...
    def _multi_edit(self, mode, value=None):
        ...
        if mode == 'insert':
            ...
            #     Note: Succeeding columns' max offsets are detected during their preceding column's process
            for line in lines:
            ...
            for column in max_offsets:
                ...
                for line in lines:
                    ...
                    if len(lines[line]) > next_column and lines[line][next_column][1] > max_offsets[next_column]:
    ...
    def _get_logical_line_offset(self, pos, phy_offset):
        ...
        # pos is simply a text iter in the line (does not mark the actual offset which is phy_offset)
        pos = pos.copy()
        pos.set_line_offset(0)
        phy_i = 0
        tab_width = self._view.get_tab_width()
    ...
    def _auto_increment(self, value):
        """ Parse an auto-incr command and return the list of values. """
        if len(value) < 1:
            return ()
        
        # Number 
        if value[0] == 'num' and len(value) == 3:
            i = float(value[1])
            result = []
            for mark in self._marks:
                str_i = str(i)
        ...
        # Alphabet
        elif value[0] == 'abc' and len(value) == 2 and value[1] in ('a', 'A', 'z', 'Z'):
            if value[1].islower():
    ...
    def _vertical_cursor_nav(self, down, to_line_end=False, edit_marks=True):
        ...
        if self._line_offset_mem is None and not to_line_end:
            self._line_offset_mem = self._get_physical_line_offset(self._buffer.get_insert())
        
        # Vertical mark memory (vertical)
        if edit_marks and (self._vert_mark_mem is None or self._vert_mark_mem[1] != to_line_end):
            self._vert_mark_mem = (pos.get_line(), to_line_end)
        
        # Prevent mid-line marks when in end-line mode
        no_forward = False
        if to_line_end and not pos.ends_line():
            if down:
                no_forward = True
        else:
            if edit_marks:
        ...
        if to_line_end:
            ...
            # Reset the line offset since it will now be invalid
            self._line_offset_mem = None
        else:
            log_offset = self._get_logical_line_offset(pos, self._line_offset_mem)
    ...
    def _clear_marks(self):
        ...
        for mark in self._marks:
            mark.set_visible(False)  # Convenient way of visually removing without redrawing the entire window
            self._buffer.delete_mark(mark)
        self._marks = []
        self._mark_i = 0
